...
long=crossover(sma1,e1)
exit=crossunder(sma1,e1)


strategy.entry("WT Cross", strategy.short, when=long and window())
strategy.close("WT Cross", when=exit)


stoploss=input(0.2)
takeprofit=input(0.1)

strategy.exit("short exit", "WT Cross", profit=close*(takeprofit/100)/ syminfo.mintick, loss=close*(stoploss/100) / syminfo.mintick)